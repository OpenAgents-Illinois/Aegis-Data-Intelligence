...
// Full incident detail page — shows diagnosis, blast radius, remediation,
// and the structured report (executive summary, anomaly details, timeline).
// Fetches both the incident object (from Zustand store) and the report
// (a separate /report endpoint that may return 204 if not yet generated).
export default function IncidentDetail() {
  ...
  // Extract incident ID from URL params (e.g. /incidents/3)
  const { id } = useParams<{ id: string }>();
  const navigate = useNavigate();

  // `current` is the single incident loaded into the store for detail view
  const { current: incident, fetchIncident, loading } = useIncidentStore();
  const [dismissReason, setDismissReason] = useState("");
  const [showDismiss, setShowDismiss] = useState(false);

  // Report is a separate structured object (IncidentReport) with executive
  // summary, anomaly details, root cause, blast radius, actions, and timeline.
  // Many UI sections below are gated on `report` being non-null.
  const [report, setReport] = useState<IncidentReport | null>(null);

  // Fetch both the incident and its report when the ID changes
  useEffect(() => {
    if (id) {
      fetchIncident(Number(id));
      getIncidentReport(Number(id))
        .then(setReport)
        .catch(() => setReport(null)); // 204 or error → no report
    }
  }, [id, fetchIncident]);

  // Show loading state until the incident object arrives from the API
  if (loading || !incident) {
    return (
  ...
  // Approve marks incident as "resolved" and re-fetches to update UI
  const handleApprove = async () => {
    await approveIncident(incident.id);
    fetchIncident(incident.id);
  };

  // Dismiss requires a reason; sets status to "dismissed"
  ...
  // diagnosis contains root_cause, root_cause_table, confidence, severity,
  // blast_radius[], and recommendations[] — populated by the Architect agent
  const diagnosis = incident.diagnosis;

  // Only show Approve/Dismiss buttons for active (non-terminal) incidents
  ...
  return (
    <div className="max-w-4xl space-y-6">
      ...
      <div className="flex items-center justify-between">
        ...
        {isActive && (
          <div className="flex gap-2">
            ...
            >
              {["high", "critical"].includes(incident.severity) ? "Approve & Fix" : "Approve"}
            </button>
            <button
      ...
      {/* Executive Summary — only rendered when the structured report exists.
          The report is generated by the ReportGenerator agent after the full
          pipeline (Sentinel → Orchestrator → Architect → Executor) completes. */}
      {report && (
      ...
      {/* Root Cause Analysis — from the Architect agent's GPT-4 diagnosis
          (or deterministic fallback). Shows what broke, where, and confidence. */}
      {diagnosis && (
        <section className="bg-white border border-gray-200 rounded-lg p-6 space-y-4">
          <h3 className="text-xs font-semibold text-gray-500 uppercase tracking-wider">
            Root Cause Analysis
      ...
      {/* Blast Radius — interactive graph showing the source table and all
          downstream tables affected by this anomaly. Uses BlastRadiusGraph
          component (XY Flow DAG visualization). */}
      ...
      {/* Recommended Actions — from diagnosis.recommendations[]. Each action
          has a type label, description, priority, and optional SQL to execute.
      ...
      {/* Timeline — chronological event log from the report showing when each
          pipeline stage ran: detection → incident creation → analysis → remediation */}
      ...
      {/* Remediation Summary — high-level text summary from the Executor agent
          describing what needs to happen to resolve this incident */}
